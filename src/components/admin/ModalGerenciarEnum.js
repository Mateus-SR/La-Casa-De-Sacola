    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm z-[130]"
        >
          ...
          >
            ...
            <Tabs.Content value="novo" className="outline-none">
              <form
                onSubmit={enum_.handleAdicionarValorEnum}
                className="flex flex-col gap-4"
              >
                <input
                  type="text"
                  placeholder={
                    enum_.enumAtual === "tipo"
                      ? "Ex: Papel, Plástico..."
                      : "Ex: 20x30, 30x40..."
                  }
                ...
                <CoresMaterialSection
                  ...
                  coresSelecionadas={cores.obterCoresSelecionadasDoMaterial(
                    enum_.novoValorEnum
                  )}
                  onAbrirFormulario={() =>
                    cores.setModalEditarCoresAberto(true)
                  }
                  onToggleCor={(cor) =>
                    cores.handleToggleSelecaoCor(enum_.novoValorEnum, cor)
                  }
                />
                ...
                <div className="flex gap-3">
                  <Dialog.Close asChild>
                    <button
                      type="button"
                      className="flex-1 p-3 bg-gray-100 hover:bg-gray-200 rounded-xl font-bold transition"
                    >
                      Cancelar
                    </button>
                  </Dialog.Close>
            ...
            <Tabs.Content
              value="gerenciar"
              className="outline-none flex flex-col gap-4"
            >
              <Select.Root
                onValueChange={(nomeSelecionado) => {
                  if (enum_.enumAtual === "tipo") {
                    const itemEncontrado = enum_.opcoesMaterial.find(
                      (item) => item.tipo_tip === nomeSelecionado
                    );
                    if (itemEncontrado) {
                    ...
                    const itemEncontrado = enum_.opcoesTamanho.find(
                      (item) => item.tamanho_tam === nomeSelecionado
                    );
                    if (itemEncontrado) {
                      ...
                      {(enum_.enumAtual === "tipo"
                        ...
                        return (
                          <Select.Item
                            key={itemId}
                            value={itemNome}
                            className="p-3 outline-none hover:bg-gray-100 cursor-pointer"
                          >
                      ...
                      <AlertDialog.Portal>
                        ...
                        <AlertDialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-8 rounded-2xl shadow-2xl w-full max-w-xs z-[160] outline-none">
                          ...
                          <AlertDialog.Description className="text-gray-600 text-sm mb-6 leading-relaxed">
                            Essa ação é permanente. O{" "}
                            {enum_.enumAtual === "tipo"
                              ? "material"
                              : "tamanho"}{" "}
                            selecionado deixará de estar disponível para novos
                            produtos.
                          ...
                          <div className="flex gap-3 justify-end">
                            ...
                            <AlertDialog.Action asChild>
                              <button
                                onClick={() =>
                                  enum_.handleOcultarEnum(enum_.enumEditandoId)
                                }
